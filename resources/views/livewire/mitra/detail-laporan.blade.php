<div class="bg-gray-100 min-h-screen flex flex-col items-start justify-start py-8 px-4">
    <nav class="mb-8 flex items-center space-x-2 text-lg px-3 py-1 rounded justify-start ml-32">
        ...
        <span class="text-black">›</span>
        <span class="bg-yellow-100 text-yellow-600 px-3 py-1 rounded">Edit Laporan</span>
    </nav>

    <h2 class="text-2xl font-semibold text-gray-900 mb-6 text-left ml-36">Edit Laporan</h2>
    ...
    <div class="max-w-5xl w-full mx-auto bg-white shadow-lg rounded-lg p-6 mb-28">
        <form wire:submit.prevent="update">
            ...
            <div class="flex space-x-4 overflow-x-auto p-4">
                ...
                @foreach ($existingImages as $image)
                    <div class="relative w-24 h-24 rounded-lg overflow-hidden border border-gray-300">
                        <img src="{{ asset('storage/' . $image) }}" alt="Uploaded image" class="w-full h-full object-cover">
                        <button type="button" wire:click="removeExistingImage('{{ $image }}')" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition">
                ...
                {{-- Display temporarily uploaded images --}}
            ...
            <div class="flex justify-end space-x-4">
                <button type="button" wire:click="saveDraft" class="flex items-center space-x-2 px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-lg focus:outline-none">
                    <img src="{{ asset('draft.png') }}" alt="" class="h-5 w-5">
                    <span>Simpan Sebagai Draft</span>
                </button>
                <button type="submit" class="flex items-center space-x-2 px-4 py-2 bg-[#98100f] hover:bg-red-800 text-white rounded-lg focus:outline-none">
