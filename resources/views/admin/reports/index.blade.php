...
<div class="container mx-auto p-6">
    ...
    <div class="flex justify-between mb-6 space-x-4">
        ...
        <div class="flex-grow">
            <form action="{{ route('reports.index') }}" method="GET">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Kegiatan..." class="border border-gray-300 rounded-md p-2 w-full" />
            </form>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @foreach($reports as $report)
            <div class="bg-white shadow-lg rounded-lg p-4 border border-gray-200 hover:shadow-xl transition-shadow duration-300">
                ...
                <p class="text-gray-600 text-sm mb-2">Pelapor: {{ $report->reporter_name }}</p>
    ...
    <!-- Pagination Links -->
    <div class="mt-6">
        {{ $reports->appends(['search' => request('search')])->links('pagination::tailwind') }}
    </div>
</div>
@endsection
